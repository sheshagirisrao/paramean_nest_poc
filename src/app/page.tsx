                ...
                {members.length === 0 ? (
                  ...
                  (() => {
                    ...
                    for (const m of members) {
                      if (!grouped[m.familyName]) grouped[m.familyName] = [];
                      grouped[m.familyName].push(m);
                    }
                    const families = Object.keys(grouped).sort();

                    return families.map((family) => {
                      const familyMembers = grouped[family];
                      const isEligible = familyMembers.some((m) => m.familyEligible === 1);

                      return (
                        <Fragment key={family}>
                          <tr className="bg-gradient-to-r from-[#5A3A76]/[0.04] to-[#8D5EAD]/[0.04]">
                            <td colSpan={8} className="px-6 py-2.5">
                              <div className="flex items-center gap-3">
                                <svg className="w-4 h-4 text-[#5A3A76]/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
                                </svg>
                                <span className="text-sm font-semibold text-[#5A3A76]">{family}</span>
                                <span className="text-xs text-[#7C89A6]">({familyMembers.length} member{familyMembers.length !== 1 ? "s" : ""})</span>
                          ...
                          {familyMembers.map((row) => (
                            <tr key={row.id} className="hover:bg-[#5A3A76]/[0.02] transition-colors border-b border-[#F0EBF5]">
                              <td className="px-6 py-3.5 text-sm font-medium text-[#1A2534] pl-12">{row.memberName}</td>
